...
```{r}
pacman::p_load(tidyverse,psych,arsenal,corrplot,janitor,pander,knitr,lavaan,car,DescTools)
...
dataset <-                 # nome atribuído ao dataframe
  read.csv(                # função da base para importar arquivos csv
    "dataset_mapfre.csv",  # nome do arquivo csv
    encoding="UTF-8",      # codificação para ler cedilhas, til, etc...
    stringsAsFactors=TRUE) # transformar variáveis string em fator
...
[Clique aqui](http://www.stat.columbia.edu/~tzheng/files/Rcolor.pdf) para baixar um arquivo com os nomes das cores do R... e [aqui](https://coolors.co/) para acessar um site gerador de paletas de cores, por código.
Os códigos devem ser inseridos após uma hashtag (#).

Utilizaremos o pacote ggplot2, que faz parte da família tidyverse.
Esse pacote utiliza a gramática dos gráficos, que produz gráficos por camadas, sendo que todo gráfico deve conter pelo menos 3 informações:

-   banco de dados (dataframe)

-   estética (identificação das variáveis que serão usadas nos eixos x e y, agrupamentos, etc.)

-   geom (tipo de gráfico)
...
ggplot(                            # chamar o pacote ggplot2
  data = dataset,                  # informar o banco de dados
  mapping = aes(x = country)) +    # informar o mapeamento estético
  geom_bar(color = "black",        # cor da borda das barras
           fill = "dodgerblue")    # cor do preenchimento das barras


# Gráfico de barras mais complexo (4 camadas)

## Camada 1 - gráfico de barras com algumas melhorias visuais

ggplot(
  data = dataset,             # base de dados
  mapping = aes(x = country,  # variável que vai no eixo x
                y = ..prop.., # variável que vai no eixo y: proporção
                group = 1)) + # 1 indica que há um grupo só. Poderia ser por sexo.. 
  geom_bar(                 # gráfico de barras
    stat = "count",         # definir que a estatística parte da contagem
    fill = "dodgerblue4",   # cor do preenchimento das barras
...
## Camada 1
  ggplot(.,
         aes(x = "", 
             y = pct, 
             fill = country)) + # por fill dentro de aes para uma cor por barra 
  geom_col(color = "black") +
## Camada 2
  geom_text(
    aes(label = scales::percent(round(pct,3))),
    position = position_stack(vjust = 0.5)) + 
## Camada 3
...
# Mesmo gráfico anterior, mas ordenando as variáveis pelo valor da porcentagem
dataset %>%
  drop_na(bdi_class,sex) %>% 
  group_by(sex) %>% 
  count(bdi_class,sex) %>%
...
## camada 1
  ggplot() +
  geom_line(
    aes(x = reorder(bdi_class,
                    -porc), # reordenação das variáveis pelo valor da porcentagem. 
                            # O sinal de menos antes de porc faz ordem decrescente. 
                y = porc, 
                group = sex, 
                color = sex), 
            size = 1.0) +
...
ggplot() +
  ## Camada 1 - Barras
  geom_bar(                    # gráfico de barras
    data = dataset,            # definição do dataframe com os dados
    mapping = aes(x = country, # estética x e y
                  y = bai_sum),
    stat = "summary",          # função para estatísticas descritivas
    fun = mean,                # informação a ser uitlizada
    na.rm = TRUE)              # remoção de NAs
...
## dataframe com as informações que vamos inserir na camada 2 do gráfico

médias_BAI <-        # criar objeto médias_BAI (nome atribuído)
  dataset %>% 
  group_by(country) %>% 
  summarise(média = mean(bai_sum, na.rm = TRUE))

ggplot() +
  # Camada 1 - Barras com embelezamento
  geom_bar(                     # definindo que será um gráfico de barras
    data = dataset,             # definindo dataframe
    mapping = aes(x = country,  # definindo variáveis no gráfico (estética)
                  y = bai_sum), 
    stat = "summary",           # função summary para estatística
    fun = mean,                 # que informação pegar dentro de summary
    na.rm = TRUE,               # remover NAs
    fill = "dodgerblue",        # cor do preenchimento
    color = "black") +          # cor da borda
  # Camada 2 - Barra de Erro
  stat_summary(                 # sobrepondo um segundo gráfico
    data = dataset,             # definindo dataframe
    mapping = aes(x=country,    # definindo variáveis no gráfico (estética)
                  y=bai_sum),
    geom = "errorbar",          # definindo que será um gráfico de barras
    fun.data = mean_se,         # pegar a informação "erro padrão da média"
    width = 0.5) +              # largura das barras de erro
  # Camada 3 - Texto (valor das médias dentro das barras)
  geom_text(                    # inserir texto
    data = médias_BAI,          # buscar dataframe criado antes
    mapping = aes(x = country,  # definindo x e y que constam no médias_BAI 
                  y = média,
                  label = format(média,
                                 digits = 1,
                                 nsmall = 1)),
...
# Gráfico de barras: Básico + Texto + Complementos (lables, themes e coord)
...
ggplot() +
  geom_boxplot(
    ...
    mapping = aes(x = country,
                  y = bai_sum,
                  fill = country), # colocar o preenchimento dentro do geom
    show.legend = FALSE,
    na.rm = TRUE) +
### camada 2
  geom_text(
...
## Visualização somente das correlações
matriz_cruz$r
matriz_cruz$stars %>% pander()
round(matriz_cruz$r, digits = 2) %>% pander()
```
...
partial_matrix <-                        # criar objeto partial_matrix 
dataset %>% 
  dplyr::select(bai_sum,bdi_sum,age) %>% # selecionar variáveis
  drop_na()                              # eliminar missing values
...
big_five <- readRDS("big_five.rds")  # importar dataframe

bf_cor <- 
  big_five %>%                            # identificar o dataframe 
  drop_na(extr,neur,cons,amab,aber) %>%   # eliminar NAs
  dplyr::select(extr,neur,cons,amab,aber) # selecionar variáveis de interesse

# OBS.: é necessário colocar o dplyr::select() porque o pacote ppcor tem o 
# pacote MASS associado, e o MASS tem uma função select() que mascara a do tidyverse

matriz_bf <- corr.test(bf_cor)
corr.test(bf_cor$extr,bf_cor$neur)
pcor.test(bf_cor[,1],bf_cor[,2],bf_cor[,c(3)])
...
```{r}
# gráfico de dispersão de pontos (depressão x ansiedade)
...
big_five %>% 
  slice_sample(n=1000) %>% # seleção aleatória de 500 casos
  ggplot(aes(x=extr,
             y=neur)) +
  geom_jitter() +
  geom_smooth(
...
# summary(lm_ans_dep) # esse linear model será rodado posteriormente
...
ggplot(
  ...
  annotate("text",x=20,y=55,label="bai_sum = 4.36700 + 0.57206*bdi_sum")
  

# matriz de correlações usando o corrplot (BAI e BDI)
...
# matriz de correlações usando o corrplot (big five)
corrplot::corrplot(matriz_bf$r, 
                   method = "shade",
                   type = "lower",
                   order = "hclust",
                   addCoef.col = TRUE)
...
ols_regress(idade ~ extr+neur+amab+cons+aber, data = big_five_BR)

# regressão linear pelo método STEPWISE
ols_step_both_p(mod2)   # aplica a função ao modelo gerado pela função lm
```
...
big_five %>% 
  mutate(País = ifelse(país == "BR","BR","OU")) %>% 
  drop_na() %>%
...
big_five_BR_nona %>% 
  ggplot() + 
  geom_point(aes(x=gênero,
                 y=amab),
             stat = "summary",
             fun = "mean") + 
  geom_errorbar(aes(x=gênero,
                    y=amab),
                stat = "summary",
                fun.data = "mean_se",
                width = 0.3,
...
4. Utilizando o dataframe teg, verifique se há efeito de *escolaridade* sobre a *tendência ao empreendedorismo geral (teg)*
```{r}
